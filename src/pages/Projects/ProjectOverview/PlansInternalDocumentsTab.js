// frontend/src/pages/Projects/PlansInternalDocumentsTab.js
...
import {
    ...
    UncontrolledDropdown, DropdownToggle, DropdownMenu, DropdownItem, FormFeedback
} from 'reactstrap';
...
import { toast } from 'react-toastify'; // CORRECTED: Changed '=>' to 'from'
import FeatherIcon from 'feather-icons-react'; // Assuming you use this


// Specific categories for Plans/Internal Documents Tab
...
const PlansInternalDocumentsTab = () => {
    // State Initialization
    const [files, setFiles] = useState([]);
    const [modal, setModal] = useState(false); // For upload/edit modal
    const [uploadFile, setUploadFile] = useState({ name: '', category: planDocumentCategories[0], file: null });
    ...
    console.log("PlansInternalDocumentsTab: projectIdFromUrl from URL =", projectIdFromUrl);

    // Load Documents on Mount (now takes projectId and categories)
    const loadDocuments = useCallback(async () => {
        setLoading(true);
        setError(null);
        try {
            // Pass projectIdFromUrl and the specific categories to fetchDocuments
            const data = await fetchDocuments(projectIdFromUrl, planDocumentCategories);
    ...
    useEffect(() => {
        if (projectIdFromUrl) {
            ...
            setLoading(false);
            console.warn("Project ID not found in URL. Not loading documents for a specific project.");
            setFiles([]);
        }
    ...
    const toggleModal = () => {
        setModal(!modal);
        if (modal) { // If closing modal, reset states
            setUploadFile({ name: '', category: planDocumentCategories[0], file: null });
            setEditingFile(null);
    ...
    const validateUploadForm = () => {
        ...
        if (!uploadFile.category || uploadFile.category.trim() === '') errors.category = 'Category is required.';
    ...
    const handleUploadSubmit = async (e) => {
        ...
        console.log("PlansInternalDocumentsTab: handleUploadSubmit: Value of uploadFile.file before FormData:", uploadFile.file);
        ...
        if (!projectIdFromUrl) {
            toast.error('Project ID is not available. Cannot upload document without a project context.');
            return;
        }
        ...
        console.log("PlansInternalDocumentsTab: Contents of FormData before sending:");
    ...
    const handleEditSave = async (e) => {
        ...
        formData.append('name', uploadFile.name);
        formData.append('category', uploadFile.category);
        if (uploadFile.file) { // If a new file was selected during edit
    ...
    const handleStatusToggle = async (id, currentStatus) => {
        ...
        try {
            ...
            if (!updatedDoc || typeof updatedDoc.id === 'undefined' || typeof updatedDoc.status === 'undefined') {
                console.error("Error: updateDocumentStatus did not return a valid updated document with id or status.", updatedDoc);
            ...
            setFiles(prev =>
                prev.map(file =>
                    (file && file.id === updatedDoc.id) ? { ...file, status: updatedDoc.status } : file
                )
            );
    ...
    // Categorize files for display based on the *new* specific categories
    const categorizedFiles = planDocumentCategories.reduce((acc, category) => {
        acc[category] = (files && Array.isArray(files))
            ? files.filter(file => file !== null && typeof file !== 'undefined' && file.category === category)
            : [];
        return acc;
    }, {});
        ...
        <Container fluid className="p-0">
            <Card>
                <CardHeader className="d-flex justify-content-between align-items-center">
                    <h4>Plans/Internal Documents (Project ID: {projectIdFromUrl || 'N/A'})</h4>
                    <Button color="primary" onClick={() => { setEditingFile(null); toggleModal(); }}>
                        Add Document
                    </Button>
                </CardHeader>
                <CardBody>
                    {loading ? (
                        ...
                        <p className="text-center text-muted p-3">No plans/internal documents found for this project. Click "Add Document" to upload one.</p>
                    ) : (
                        Object.keys(categorizedFiles).map(category => (
                            ...
                            categorizedFiles[category].length > 0 && (
                                <div key={category} className="mb-4 mt-4">
                                    <h5 className="mb-3">{category}</h5> {/* Added mb-3 here */}
                                    <div className="table-responsive table-card">
                                        {/* Added table-layout: fixed to ensure consistent column widths */}
                                        <Table className="table-borderless align-middle mb-0" style={{ tableLayout: 'fixed', width: '100%' }}>
                                            <thead className="table-light">
                                                <tr>
                                                    {/* Explicitly setting widths for better alignment across categories */}
                                                    <th className="py-2 px-3 align-middle text-start text-nowrap" style={{ width: '5%' }}>Sr.</th>
                                                    <th className="py-2 px-3 align-middle text-start text-nowrap" style={{ width: '15%' }}>Date Upload</th>
                                                    <th className="py-2 px-3 align-middle text-start text-nowrap" style={{ width: '20%' }}>Category</th>
                                                    <th className="py-2 px-3 align-middle text-start text-nowrap" style={{ width: '30%' }}>Document Name</th>
                                                    <th className="py-2 px-3 align-middle text-start text-nowrap" style={{ width: '10%' }}>Size</th>
                                                    <th className="py-2 px-3 align-middle text-start text-nowrap" style={{ width: '10%' }}>Status</th>
                                                    <th className="py-2 px-3 align-middle text-start text-nowrap" style={{ width: "10%" }}>Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {(categorizedFiles[category] || []).length > 0 ? (
                                                    (categorizedFiles[category] || []).map((file, index) => (
                                                        <tr key={file.id}>
                                                            <td className="py-2 px-3 align-middle text-start">{index + 1}</td>
                                                            <td className="py-2 px-3 align-middle text-start">{file.date}</td>
                                                            <td className="py-2 px-3 align-middle text-start">{file.category}</td>
                                                            <td className="py-2 px-3 align-middle text-start">
                                                                <div className="d-flex align-items-center" style={{ width: '100%' }}> {/* Ensure div takes full width */}
                                                                    <div className="avatar-sm me-2 flex-shrink-0">
                                                                        <div className={`avatar-title bg-light ${file.color} rounded fs-24`}>
                                                                    ...
                                                                    <h5 className="fs-14 mb-0 text-truncate flex-grow-1">
                                                                        <Link to="#" onClick={() => handleDocumentViewDownload(file)} className="text-body">
                                                                            {file.name}
                                                                        </Link>
                                                                    </h5>
                                                                </div>
                                                            </td>
                                                            <td className="py-2 px-3 align-middle text-start">{file.size}</td>
                                                            <td className="py-2 px-3 align-middle text-start">
                                                                <Button
                                                                    size="sm"
                                                                    color={file.status === 'Verified' ? 'success' : 'warning'}
                                                                    onClick={() => handleStatusToggle(file.id, file.status)}
                                                                    className="btn-label waves-effect waves-light"
                                                                >
                                                                    <i className={`ri-checkbox-circle-line label-icon align-middle rounded-pill ${file.status === 'Verified' ? 'fs-12' : 'fs-10'}`}></i>
                                                                    {file.status}
                                                                </Button>
                                                            </td>
                                                            <td className="py-2 px-3 align-middle text-start">
                                                                ...
                                                                <Button
                                                                    ...
                                                                    size="sm"
                                                                    className="me-1"
                                                                    onClick={() => handleDocumentViewDownload(file)}
                                                                >
                                                                ...
                                                                <Button
                                                                    ...
                                                                    color="danger"
                                                                    size="sm"
                                                                    onClick={() => openDeleteConfirmModal(file)}
                                                                >
                                                    ...
                                                    <tr>
                                                        <td colSpan="7" className="text-center text-muted py-3 align-middle">No documents in this category.</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </Table>
                ...
                <Form onSubmit={editingFile ? handleEditSave : handleUploadSubmit}>
                    <ModalBody>
                        ...
                        <FormGroup>
                            ...
                            {/* Uses the specific categories for this tab */}
                            <Input
                                id="documentCategory"
                                type="select"
                            ...
                            >
                                ...
                                {planDocumentCategories.map(cat => (
                        ...
                        <FormGroup className="mt-3">
                            ...
                            {editingFile && !uploadFile.file && <small className="form-text text-muted">Leave blank to keep existing file.</small>}
                        </FormGroup>
                    </ModalBody>
                    <ModalFooter>
            ...
            <Modal isOpen={isDeleteConfirmModalOpen} toggle={() => setIsDeleteConfirmModalOpen(false)} centered>
                <ModalHeader toggle={() => setIsDeleteConfirmModalOpen(false)}>Confirm Deletion</ModalHeader>
                <ModalBody>
                    <p>
                ...
                <ModalFooter>
                    <Button color="secondary" onClick={() => setIsDeleteConfirmModalOpen(false)}>
                        Cancel
                    </Button>
...
export default PlansInternalDocumentsTab;
